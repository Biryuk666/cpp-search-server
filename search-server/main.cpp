...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if (document_id < 0 || documents_.count(document_id) != 0) {
            throw invalid_argument("The document id must not be less than 0 and must be unique"s);
        }
...
int main() {
    ...
    try {
        search_server.AddDocument(1, "пушистый кот пу\x12шистый хвост"s, DocumentStatus::ACTUAL, {7, 2, 7});
    } catch (invalid_argument& e) {
        cout << e.what() << endl;
    }
    ...
    try {
        search_server.FindTopDocuments("пуш\x12истый"s);
    } catch (invalid_argument& e) {
        cout << e.what() << endl;
    }
    ...
    try {
        search_server.MatchDocument("пуш\x12истый"s, 0);
    } catch (invalid_argument& e) {
        cout << e.what() << endl;
    }
